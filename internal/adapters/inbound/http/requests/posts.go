...
type UpdatePost struct {
	...
	CategoryID json.RawMessage `json:"categoryId" swaggertype:"string" format:"uuid"`
	Tags       *[]string       `json:"tags"`
	// Who may comment: open, authenticated, read_only, or disabled (hides existing comments).
	CommentPolicy *string `json:"commentPolicy" binding:"omitempty,oneof=open authenticated read_only disabled" enums:"open,authenticated,read_only,disabled"`
}

// ReplacePostMedia is PATCH /api/v1/admin/posts/:id/media.
type ReplacePostMedia struct {
	EnforceCoverConsistency *bool           `json:"enforceCoverConsistency"`
...
type PostMediaItem struct {
	MediaAssetID string `json:"mediaAssetId" binding:"required,uuid"`
