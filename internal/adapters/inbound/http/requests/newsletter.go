...
type NewsletterSubscribe struct {
	...
	DisplayName       string   `json:"displayName" binding:"max=100" example:"Alex"`
	...
	TurnstileResponse string   `json:"turnstileResponse" binding:"max=2048"`
}
...
type NewsletterUnsubscribe struct {
	...
	ReasonCode string `json:"reasonCode" binding:"omitempty,oneof=too_frequent not_relevant never_signed_up other"`
...
// NewsletterIssueCreate creates an issue: draft (default), scheduled (needs sendAt), or
// queued (send now). bodyMarkdown is rendered to sanitized HTML and plain text.
type NewsletterIssueCreate struct {
	...
	BodyMarkdown string     `json:"bodyMarkdown" binding:"required,max=200000" example:"## Hello\n\nThis month..."`
	...
	SendAt       *time.Time `json:"sendAt" example:"2026-10-01T09:00:00Z"`
}
...
type NewsletterIssuePatch struct {
	...
	BodyMarkdown *string    `json:"bodyMarkdown" binding:"omitempty,max=200000"`
...
type NewsletterProviderConfig struct {
	FromName            string           `json:"fromName" binding:"max=100" example:"Turahe Blog"`
	FromEmail           string           `json:"fromEmail" binding:"omitempty,max=254" example:"newsletter@example.com"`
	ReplyTo             string           `json:"replyTo" binding:"omitempty,max=254" example:"hello@example.com"`
	PostalAddress       string           `json:"postalAddress" binding:"max=500" example:"123 Example Street, Jakarta, ID"`
	ConfirmTTLHours     int              `json:"confirmTtlHours" binding:"required,min=1,max=168" example:"48"`
	DoubleOptInRequired *bool            `json:"doubleOptinRequired" binding:"required" example:"true"`
