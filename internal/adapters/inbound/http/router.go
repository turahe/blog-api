...
func handlerFor(deps Dependencies) func(v1.Route) gin.HandlerFunc {
	...
	if deps.Posts != nil {
		...
		unpublishPost := adminUnpublishPostHandler(deps.Posts)
		archivePost := adminArchivePostHandler(deps.Posts)
		...
		if deps.RBAC != nil {
			...
			implemented["admin.posts.unpublish"] = chain(requirePermission(deps.RBAC, "post.publish"), unpublishPost)
			implemented["admin.posts.archive"] = chain(requirePermission(deps.RBAC, "post.archive"), archivePost)
			...
			editorGate := requireRoles(deps.Roles, "admin", "editor")
			authorGate := requireRoles(deps.Roles, "admin", "editor", "author")
			implemented["admin.posts.list"] = chain(authorGate, listPosts)
			implemented["admin.posts.create"] = chain(authorGate, createPost)
			implemented["admin.posts.publish"] = chain(editorGate, publishPost)
			implemented["admin.posts.unpublish"] = chain(editorGate, unpublishPost)
			implemented["admin.posts.archive"] = chain(editorGate, archivePost)
			implemented["admin.posts.update"] = chain(authorGate, updatePost)
			implemented["admin.posts.media.replace"] = chain(authorGate, replaceMedia)
		} else {
			...
			implemented["admin.posts.unpublish"] = unpublishPost
			implemented["admin.posts.archive"] = archivePost
